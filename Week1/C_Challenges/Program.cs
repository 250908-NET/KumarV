...
app.MapGet("/divide/{a}/{b}", (double a, double b) =>
{
    object result;
    ...
    {
        result = new { operation = "divide", inputa = a, inputb = b, quotient = a / b };
    }
...
app.MapGet("/text/palindrome/{text}", (string text) =>
{
    ...
    } else {return new{isPalindrome = false};
    }
...
app.MapGet("/numbers/prime/{count}", (int count) =>
{
    ...
    if(count <= 1) {return new {isPrime = "False"};}
    if(count == 2) {return new {isPrime = "True"};}
    if(count % 2 == 0) {return new {isPrime =  "False"};}

    for(int i = 3; i <= Math.Sqrt(count); i+= 2){
        if(count % i == 0) {return new {isPrime = "False"};}
    }
...
app.MapGet("/numbers/fibonacci/{count}", (int count) =>
{
    ...
    if(count == 0) {return new {Fibonacci = "0"};}
    if(count == 1) {return new {Fibonacci = "0 1"};}
    int temp1 = 0;
    int temp2 = 1;
    int newNum = 1;
